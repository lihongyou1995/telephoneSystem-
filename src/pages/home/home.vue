    ...
    <div v-if="roleId != 3">
      <div class="hoemtype">
        <div class="homediv bg1" @click="toform" style="cursor:pointer">
          <p>
            <span class="numcont">{{countForm}}</span>
            <span class="textfont">个</span>
          </p>
          <p>
        ...
        <div class="homediv bg2" @click="toform" style="cursor:pointer">
          <p>
            <span class="numcont">{{notSuccessForm}}</span>
            <span class="textfont">个</span>
          </p>
          <p>
        ...
        <div class="homediv bg3" @click="totask" style="cursor:pointer">
          <p>
            <span class="numcont">{{countTask}}</span>
            <span class="textfont">个</span>
          </p>
          <p>
        ...
        <div class="homediv bg4" @click="totask" style="cursor:pointer">
          <p>
            <span class="numcont">{{notSuccessTask}}</span>
            <span class="textfont">个</span>
          </p>
          <p>
      ...
      <div v-if="!istaskview && !isformview">
        <div class="topdiv">
          <div class="left">
            ...
            <div class="datalist">
              <ul>
                <li
                  v-for="(item,index) in taskListall"
                  :key="index"
          ...
          <div class="right" style>
            ...
            <div class style>
              <ul class="msgcontent" style="overflow-y: scroll;height:220px;padding-bottom:30px">
                <li
                  style="font-size:18px;"
                  v-for="(item,index) in msgtableData"
                  :key="index"
                >{{item.message}}</li>
              </ul>
            </div>
        ...
        <div class="bootmlist">
          <div v-for="(item,index) in formAttributeList" :key="index">
            ...
            <ul v-if="index == 0" style="overflow-y:scroll;height:215px">
              <li
                style="font-size:18px;"
                v-for="(item,index) in tableData1"
                :key="index"
                @click="clickform(item)"
              >{{item.formName}}</li>
            </ul>
            <ul v-if="index == 1" style="overflow-y:scroll;height:215px">
              <li
                style="font-size:18px;"
            ...
            <ul v-if="index == 2" style="overflow-y:scroll;height:215px">
              <li
                style="font-size:18px;"
...
export default {
  ...
  methods: {
    ...
    getallFormNewsfn() {
      this.$http
        .allFormNews(`userId=${this.userId}&pageNum=1&pageSize=1000000&type=1`)
        .then(res => {
          console.log(res);
          if (res.code == 200) {
            this.msgid = [];
    ...
    getformAndTaskStatisticsfn() {
      this.$http.formAndTaskStatistics(`userId=${this.userId}`).then(res => {
        ...
        if (res.code == 200) {
          ...
          this.successFormRate =
            (res.data.successFormRate * 10000).toFixed(2) / 100;
          this.successTask = res.data.successTask;
          this.successTaskRate =
            (res.data.successTaskRate * 10000).toFixed(2) / 100;
        } else {
    ...
    queryTaskListfn() {
      ...
      new Promise((resolve, reject) => {
        _this.$http
          .queryTaskList(
            `unitId=${_this.unitId}&userId=${_this.userId}&roleId=${_this.roleId}&pageNum=1&pageSize=10000000`
          )
          .then(res => {
            console.log("----------");
        ...
        .then(val => {
          return new Promise((resolve, reject) => {
            // 表单--专项任务
            _this.$http
              .getFormObj(`userId=${_this.userId}&formState=专项任务`)
              .then(res => {
                console.log("-------------");
        ...
        .then(val => {
          ...
          // });
          // 排序 -- 倒序
          for (var i = _this.taskList.length - 1; i > 0; i--) {
            for (var j = 0; j < i; j++) {
              if (new Date(_this.taskList[j].createTime).getTime() < new Date(_this.taskList[j+1].createTime).getTime()) {
                [_this.taskList[j], _this.taskList[j + 1]] = [_this.taskList[j + 1], _this.taskList[j]];
              }
            }
          }
          console.log("999999999999999999")
          console.log(_this.taskList)
          return _this.taskListall = _this.taskList
        });
    },
    // 查询表单属性
    ...
    getformdata() {
      if (this.formAttributeList[0].attribute) {
        this.$http
          .getFormObj(
            `userId=${this.userId}&formState=${this.formAttributeList[0].attribute}`
          )
          .then(res => {
            console.log(res);
            return res.code == 200
      ...
      if (this.formAttributeList[1].attribute) {
        this.$http
          .getFormObj(
            `userId=${this.userId}&formState=${this.formAttributeList[1].attribute}`
          )
          .then(res => {
            console.log(res);
            return res.code == 200
      ...
      if (this.formAttributeList[2].attribute) {
        this.$http
          .getFormObj(
            `userId=${this.userId}&formState=${this.formAttributeList[2].attribute}`
          )
          .then(res => {
            console.log(res);
            return res.code == 200
    ...
    getDataListfn() {
      this.$http
        .getDataList(
          `unitId=${this.unitId}&userId=${this.userId}&roleId=${this.roleId}&taskId=${this.taskId}&taskName=${this.taskName}&from='正式`
        )
        .then(res => {
          console.log(res);
