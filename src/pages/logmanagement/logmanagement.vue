    ...
    <div class="check1">
      <el-drawer direction="rtl" title="日志操作详情" center :visible.sync="viewDetailVisible" size="55%">
        ...
        >
          <el-table-column
            :resizable="false"
            v-for="(item, index) in viewDetailheaderData"
            :key="index"
            :label="item"
            :prop="item"
            align="center"
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column
            ...
            max-width="300"
          ></el-table-column>
          <el-table-column prop="operation" label="操作" align="center" max-width="100"></el-table-column>
          ...
          <el-table-column label="操作详情" align="center" fixed="right" width="160">
            <template slot-scope="scope">
              <el-button
                ...
                v-if="scope.row.dataMessage.length"
              >查看操作详情</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    viewDetail(row) {
      ...
      this.type = row.type; // 判断是表单还是任务
      this.objId = row.objId; // 表单或者任务ID
      // console.log(this.objId);
      // console.log(this.type);
      if (that.type == "表单") {
        that.$http.gettableheader(`formId=${that.objId}`).then(res => {
          let datas = [];
          res.data.forEach(item => {
            console.log(item.propertyName);
            datas.push(item.propertyName);
          });
          this.viewDetailheaderData = datas; // 表头
          console.log(that.viewDetailheaderData.length);
          let jsonData = JSON.parse(row.dataMessage);
          console.log(jsonData)
          that.viewDetailtableData=jsonData
          console.log(that.viewDetailtableData);
        });
      } else if (that.type == "任务") {
        that.$http.getTaskProperty(`taskId=${that.objId}`).then(res => {
          let datas = [];
          res.data.forEach(item => {
