      ...
      width="150"
       ...
       width="120"
           ...
           width="120"
            ...
            <el-table-column :resizable="false" v-if="unitId != 0 && roleId == 1" label="操作"  width="150" align="center" fixed="right">
                <template slot-scope="scope">
                    <el-button type="primary" size="mini" @click="vieworedit(scope.row,1)">查看</el-button>
                    <el-button v-if="userId == scope.row.createUser" type="danger" size="mini" @click="vieworedit(scope.row,2)">删除</el-button>
                </template>
            </el-table-column>
            <el-table-column :resizable="false" v-if="roleId == 2 || roleId == 3" label="操作"  width="100" align="center" fixed="right">
                <template slot-scope="scope">
                    <el-button type="primary" size="mini" @click="vieworedit(scope.row,1)">查看</el-button>
                </template>
            </el-table-column>
            ...
            <div>
              <p style="margin-bottom:10px">单位：</p>
                <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
                    <el-checkbox v-for="(city,index) in cities" :label="city.managerId" :key="index">{{city.title}}</el-checkbox>
                  </el-checkbox-group>
              <p style="margin:10px 0">填报员：</p>
                  <el-checkbox-group v-model="distributeuser" @change='distributeuserfn'>
                      <el-checkbox v-for="(item,index) in distributor" :key="index" :label="item.userId">{{item.account}}---{{item.userName}}</el-checkbox>
                </el-checkbox-group>
            </div>
              <div class="quexiafa">
...
methods: {
    ...
    // 获取任务列表数据
    queryTaskListfn(){
        this.$http.queryTaskList(`unitId=${this.unitId}&userId=${this.userId}&roleId=${this.roleId}&pageNum=${this.pageIndex}&pageSize=${this.pageSize}`).then(res =>{
    ...
    // 获取用户任务列表数据
    queryuserTaskListfn(){
      this.$http.getTaskTab(`userId=${this.userId}&atrName=${this.taskState}`).then(res =>{
    ...
    countTime(){
        ...
        this.tableData.forEach(res =>{
            if(res.createUserId == this.userId || res.endTime == null) return res.endTimes = '无';
    ...
    handleSelectionChange(list) {
      ...
      //   console.log(item.stateCnName)
      //   if(item.stateCnName == '待上报' || item.stateCnName == '待下发'){
      //     this.istuisong = true;
      //   }else{
    ...
    lowerhair(i){
      if(this.multipleSelection.length == 0) return this.$message('请选择任务');
      if(this.multipleSelection.length >1) return this.$message('每次只能选择一个任务');
      this.dialogFormVisible4 = true;
      // 请求下发单位
      this.$http.queryTree3(`/unit/queryTree3?unitId=${this.unitId}`).then(res =>{
                  console.log(res)
                  this.cities = [];
                  if(res.code == 200){
      ...
      // 请求下发填报员
     this.$http.getReportersByUnitId(`/user/getReportersByUnitId?unitId=${this.unitId}`).then(res =>{
    ...
    datetimefn(date){
        console.log(date)
         if(date.getTime() >= Date.now()){
            ...
            this.lowerhairtime = year + '-' + month + '-' + day + ' ' + hour + ':' + minute + ':' + second;
    ...
    confirmthenextissue(){
        ...
        if(this.textarea1.indexOf(',') != -1 || this.textarea1.indexOf(':') != -1 ) return this.$message('备注不能出现英文状态下的逗号或者冒号');
        if(this.lowerhairtime == '') return this.$message('请选择截止时间');
        this.dialogFormVisible4 = false;
        console.log(`任务ID${this.multipleSelectionid.toString()}-------任务名${this.multipleSelectionname.toString()}----下发城市${this.userIds.toString()}`)
        this.$http.newTaskpublish({
            unitId:this.unitId,
          ...
          console.log(res)
          this.queryTaskListfn();
          return res.code == 200 ? this.$message.success(res.message) : this.$message(res.data || res.message);              
        })
          ...
          this.$confirm('是否继续删除这条任务记录?', '提示', {
              ...
              type: 'warning'
                ...
                this.$http.deleteTask({
                  ...
                  console.log(res)
                  this.queryTaskListfn(); 
                  res.code == 200 ? this.$message.success('删除成功') : this.$message(res.data || res.message);
                })
            }).catch(() => {});
        }else if(type == 5){
            this.$router.push({
